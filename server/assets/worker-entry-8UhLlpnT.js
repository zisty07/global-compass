...
function resolveEnvRoot() {
  ...
  if (existsSync(resolve(cwd2, ".env"))) return cwd2;
  try {
...
function loadServerEnv() {
  ...
  try {
    ...
    mainExports.config({ path: resolve(root, ".env") });
    mainExports.config({ path: resolve(root, ".env.local"), override: true });
  } catch {
  }
}
loadServerEnv();
let bindings;
function setWorkerEnv(env2) {
  if (!env2 || typeof env2 !== "object") return;
  bindings = env2;
...
function readWorkerEnvString(key) {
  const bound = bindings?.[key];
  if (typeof bound === "string" && bound.trim()) return bound.trim();
  const fromProcess = process.env[key];
  return typeof fromProcess === "string" && fromProcess.trim() ? fromProcess.trim() : void 0;
}
...
async function getServerEntry() {
  if (!serverEntryPromise) {
    serverEntryPromise = import("./server-BwRcFVCS.js").then((n) => n.a5).then(
      (m) => m.default ?? m
    );
  }
